...
    long long solve(int i, int j, vector<int>& 
robot, vector<vector<int>>& factory){
        if(i == robot.size()) return 0;
        if(j == factory.size()) return 1e18;
        if(dp[i][j] != -1) return dp[i][j];

        long long res = solve(i, j+1, robot, 
factory);

        long long cost = 0;
        for(int k = 0; k < factory[j][1] && i + 
k < robot.size(); k++){
            cost += abs(robot[i+k] - factory[j]
[0]);
            res = min(res, cost + solve(i+k+1, j
+1, robot, factory));
        }
